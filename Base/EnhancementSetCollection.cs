...
namespace mrbBase
{
    public class EnhancementSetCollection : List<EnhancementSet>
    {
        ...
        {
            if ((iSet < 0) | (iSet > DatabaseAPI.Database.EnhancementSets.Count - 1)) return string.Empty;

            string str2;
            if (DatabaseAPI.Database.EnhancementSets[iSet].LevelMin == DatabaseAPI.Database.EnhancementSets[iSet].LevelMax)
                str2 = RTF.Color(RTF.ElementID.Invention) + RTF.Bold(RTF.Underline(
            ...
            var str3 = str2 + RTF.Color(RTF.ElementID.Text);
            for (var index = 0; index <= DatabaseAPI.Database.EnhancementSets[iSet].Bonus.Length - 1; ++index)
            {
                ...
                if (DatabaseAPI.Database.EnhancementSets[iSet].Bonus[index].PvMode == Enums.ePvX.PvP)
                    effectString += " (PvP)";
                if ((enhCount >= DatabaseAPI.Database.EnhancementSets[iSet].Bonus[index].Slotted) &
                    ...
                    str3 = str3 + RTF.Crlf() +
                           RTF.Bold(RTF.Color(RTF.ElementID.Text) + "  " +
                                    DatabaseAPI.Database.EnhancementSets[iSet].Bonus[index].Slotted + " Slotted: ") +
                           RTF.Color(RTF.ElementID.Invention) + effectString + RTF.Color(RTF.ElementID.Text);
                else
                    str3 = str3 + RTF.Crlf() +
                           RTF.Bold(RTF.Color(RTF.ElementID.Text) + "  " +
                                    DatabaseAPI.Database.EnhancementSets[iSet].Bonus[index].Slotted + " Slotted: ") +
                           RTF.Color(RTF.ElementID.Faded) + effectString + RTF.Color(RTF.ElementID.Text);
            }
            ...
            for (var index = 0; index <= DatabaseAPI.Database.EnhancementSets[iSet].SpecialBonus.Length - 1; ++index)
            {
                ...
                if (!string.IsNullOrEmpty(effectString))
                    str3 = str3 + RTF.Crlf() + RTF.Color(RTF.ElementID.Enhancement) +
                           RTF.Bold("  " + DatabaseAPI.Database
                               .Enhancements[DatabaseAPI.Database.EnhancementSets[iSet].Enhancements[index]].Name + ": ") +
                           RTF.Color(RTF.ElementID.Faded) + effectString + RTF.Color(RTF.ElementID.Text);
            }
