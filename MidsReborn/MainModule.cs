...
namespace Mids_Reborn
{
    public sealed class MainModule
    {
        public class MidsController
        {
            ...
            public static void LoadData(ref frmInitializing iFrm, string path, bool legacy = false)
            {
                ...
                if (!DatabaseAPI.LoadMainDatabase(path, legacy))
                {
                ...
                DatabaseAPI.LoadEnhancementDb(path, legacy);
                DatabaseAPI.LoadOrigins(path);
                DatabaseAPI.LoadSetTypeStrings(path);

                iFrm?.SetMessage("Loading Recipe Database...");
                DatabaseAPI.LoadSalvage(path, legacy);
                DatabaseAPI.LoadRecipes(path, legacy);
