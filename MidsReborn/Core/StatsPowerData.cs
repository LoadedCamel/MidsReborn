...
using System.Linq;
using static Mids_Reborn.Forms.WindowMenuItems.frmStats;
...
namespace Mids_Reborn.Core
{
    public class StatsPowerData
    {
        ...
        public void SetTotalsGroup(TotalStatsGroups group, TotalStat[] values)
        {
            switch (group)
            {
                case TotalStatsGroups.Defense:
                    Totals.Defense = [];
                    Totals.Defense = Totals.Defense.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.Resistance:
                    Totals.Resistance = [];
                    Totals.Resistance = Totals.Resistance.Concat(values).ToArray();
                    break;
                
                case TotalStatsGroups.Health:
                    Totals.Health = [];
                    Totals.Health = Totals.Health.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.Endurance:
                    Totals.Endurance = [];
                    Totals.Endurance = Totals.Endurance.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.Movement:
                    Totals.Movement = [];
                    Totals.Movement = Totals.Movement.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.Stealth:
                    Totals.Stealth = [];
                    Totals.Stealth = Totals.Stealth.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.MiscBuffs:
                    Totals.MiscBuffs = [];
                    Totals.MiscBuffs = Totals.MiscBuffs.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.StatusProtection:
                    Totals.StatusProtection = [];
                    Totals.StatusProtection = Totals.StatusProtection.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.StatusResistance:
                    Totals.StatusResistance = [];
                    Totals.StatusResistance = Totals.StatusResistance.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.DebuffResistance:
                    Totals.DebuffResistance = [];
                    Totals.DebuffResistance = Totals.DebuffResistance.Concat(values).ToArray();
                    break;

                case TotalStatsGroups.Elusivity:
                    Totals.Elusivity = [];
                    Totals.Elusivity = Totals.Elusivity.Concat(values).ToArray();
                    break;
            }
        }
        ...
        public static TotalStats GetTotals(bool forcePvP = false)
        {
            ...
            var pvpElusivity = forcePvP ? 0.4f : 0;
        ...
        private PowerValueInfo[]? SetGraphValues(bool baseOverride, Enums.GraphStyle graphStyle, DisplayMode statDisplayed)
        {
            ...
            string[] statLabels = statDisplayed switch
            {
                ...
                DisplayMode.StatusProtection or DisplayMode.StatusResistance => ["Held", "Stunned", "Sleep", "Immobilized", "Knockback", "Repel", "Confused", "Taunt", "Placate", "Teleport"],
                ...
                _ => ["Smashing", "Lethal", "Fire", "Cold", "Energy", "Negative", "Psionic", "Melee", "Ranged", "AoE"]
            };

            var ret = new List<PowerValueInfo>();
            var num1 = 1f;
            for (var index = 0; index < statGroup.Length; index++)
            {
                var nBase = statGroup[index].BaseValue;
                var nEnh = statGroup[index].EnhValue ?? statGroup[index].BaseValue; // ??
                var nUncapped = statGroup[index].UncappedValue ?? statGroup[index].BaseValue;
                ...
                switch (statDisplayed)
                {
                    case DisplayMode.Defense:
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName} Defense: {nBase:##0.#}%"
                            : $"{displayName} Defense: {nEnh:##0.#}%";
                        break;

                    case DisplayMode.Resistance:
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName} Resistance: {nBase:##0.#}%"
                            : $"{displayName} Resistance: {nEnh:##0.#}%";
                        break;
                    ...
                    case DisplayMode.HealthEndurance when index == 2: // Absorb
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName}: {nBase:###0.#}" // Also show % of base HP here
                            : $"{displayName}: {nEnh:###0.#}";
                        valueSuffix = "";
                        break;
                    ...
                    case DisplayMode.MovementStealth when index is < 2 or 3: // Run Speed, Jump Speed, Fly Speed
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName}: {nBase:###0.##} mph"
                            : $"{displayName}: {nEnh:###0.##} mph";
                        valueSuffix = "mph";
                        break;

                    case DisplayMode.MovementStealth when index is 2 or > 3: // Jump height
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName}: {nBase:###0.##} ft"
                            : $"{displayName}: {nEnh:###0.##} ft";
                        valueSuffix = "ft";
                        break;
                    ...
                    case DisplayMode.StatusProtection:
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"{displayName} Protection: {nBase:###0.##}"
                            : $"{displayName} Protection: {nEnh:###0.##}";
                        valueSuffix = "";
                        break;
                    ...
                    case DisplayMode.DebuffResistance:
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"Resistance to {displayName}: {nBase:####0.##}%"
                            : $"Resistance to {displayName}: {nEnh:####0.##}%";
                        break;

                    case DisplayMode.Elusivity:
                        tip = graphStyle == Enums.GraphStyle.baseOnly
                            ? $"Elusivity ({displayName}): {nBase:####0.##}%"
                            : $"Elusivity ({displayName}): {nEnh:####0.##}%";
                        break;
                }
                ...
                ret.Add(new PowerValueInfo
                {
                    PowerName = displayName,
                    PowerTaken = true, // dummy
                    BaseValue = nBase,
                    EnhValue = nEnh,
                    UncappedValue = nUncapped,
                    UnitSuffix = valueSuffix,
                    Tip = tip
                });
            }
