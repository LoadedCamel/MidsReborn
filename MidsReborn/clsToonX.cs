...
namespace Mids_Reborn
{
    public class clsToonX : Character
    {
        ...
        private IPower GBPA_ApplyPowerOverride(ref IPower ret)
        {
            ...
            {
                ret.AppliedPowersOverride = true;
                return ret;
            }
        ...
        private bool GBPA_AddSubPowerEffects(ref IPower ret, int hIDX)
        {
            ...
            var effectsList = ((IEffect[]) ret.Effects.Clone()).ToList();
            foreach (var sp in CurrentBuild.Powers[hIDX].SubPowers.Where(sp => sp is {nIDPower: > -1, StatInclude: true}))
            {
                ...
                {
                    effectsList.Add((IEffect)DatabaseAPI.Database.Power[sp.nIDPower].Effects[index2].Clone());
                    effectsList[^1].Absorbed_EffectID = index2;
                    effectsList[^1].Absorbed_Effect = true;
                    effectsList[^1].Absorbed_Power_nID = sp.nIDPower;
                    effectsList[^1].Absorbed_PowerType = DatabaseAPI.Database.Power[sp.nIDPower].PowerType;
                }
            }

            ret.Effects = effectsList.ToArray();
            ret.AppliedSubPowers = true;
        ...
        private static void HandleGrantPowerIncarnate(ref IPower powerMath, IEffect effect1, IReadOnlyList<IPower> buffedPowers, int effIdx, Archetype? at, int hIDX)
        {
            ...
            if (hIDX <= -1) return;
            {
                ...
                for (var index2 = buffedPowers[hIDX].Effects.Length; index2 < buffedPowers[hIDX].Effects.Length; index2++)
                {
        ...
        private void GBPA_ApplyIncarnateEnhancements(ref IPower powerMath, int hIDX, IPower? power, bool ignoreED, ref Enums.eEffectType effectType)
        {
            ...
            if (!powerMath.Slottable & powerMath.GetPowerSet()?.SetType != Enums.ePowerSetType.Redirect)
            {
                return;
            }
        ...
        private static bool GBPA_Pass2_ApplyED(ref IPower powerMath)
        {
            ...
            foreach (var eff in powerMath.Effects)
            {
                ...
                for (var index2 = 0; index2 < Enum.GetValues(Enums.eEffectType.None.GetType()).Length; index2++)
                {
                ...
                switch (eEffectType)
                {
                    ...
                    default:
                        for (var index2 = 0; index2 < powerMath.Effects.Length; index2++)
                        {
                            ...
                            {
                                case Enums.eEffectType.Damage:
                                    for (var index3 = 0; index3 < Enum.GetValues(powerMath.Effects[index2].DamageType.GetType()).Length; index3++)
                                    {
                                    ...
                                    break;
                                case Enums.eEffectType.Defense:
                                    for (var dmgTypeIndex = 0; dmgTypeIndex < Enum.GetValues(powerMath.Effects[index2].DamageType.GetType()).Length; dmgTypeIndex++)
                                    {
                                    ...
                                    break;
                                case Enums.eEffectType.Mez:
                                    for (var index3 = 0; index3 < Enum.GetValues(powerMath.Effects[index2].MezType.GetType()).Length; index3++)
                                    {
                                ...
                                default:
                                    var effect = powerMath.Effects[index2];
                                    if (effect.EffectType == Enums.eEffectType.Enhancement & effect.ETModifies is Enums.eEffectType.SpeedRunning or Enums.eEffectType.SpeedJumping or Enums.eEffectType.JumpHeight or Enums.eEffectType.SpeedFlying)
                                    {
                            ...
                            powerMath.Effects[index2].Math_Duration += fxDuration;
                        }
        ...
        {
            return !(iPower < 0 | _buffedPowers.Length - 1 < iPower) ? _buffedPowers[iPower] : null;
        }
