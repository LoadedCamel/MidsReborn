...
namespace Mids_Reborn
{
    public class ClsOutput
    {
        ...
        private string buildSetBonusListLong()
        {
            ...
            var num1 = MidsContext.Character.CurrentBuild.SetBonuses.Count - 1;
            for (var index1 = 0; index1 <= num1; ++index1)
            {
                var num2 = MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo.Length - 1;
                for (var index2 = 0; index2 <= num2; ++index2)
                {
                    if (MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].Powers.Length <= 0)
                    {
                        continue;
                    }

                    var setInfo = MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo;
                    var index3 = index2;
                    var str2 = str1 +
                               formatColor(
                    ...
                    if (MidsContext.Character.CurrentBuild
                            .Powers[MidsContext.Character.CurrentBuild.SetBonuses[index1].PowerIndex].NIDPowerset > -1)
                    {
                        str2 = str2 + LineBreak() + formatColor(
                            "(" + DatabaseAPI.Database
                                .Powersets[
                                    MidsContext.Character.CurrentBuild
                                        .Powers[MidsContext.Character.CurrentBuild.SetBonuses[index1].PowerIndex]
                                        .NIDPowerset]
                                .Powers[
                                    MidsContext.Character.CurrentBuild
                                        .Powers[MidsContext.Character.CurrentBuild.SetBonuses[index1].PowerIndex]
                    ...
                    var iText = "";
                    var num3 = MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].SlottedCount - 2;
                    for (var index4 = 0; index4 <= num3; ++index4)
                    {
                        var str3 = iText + ListItemOn();
                        var flag = false;
                        var str4 = "  " + DatabaseAPI.Database
                            .EnhancementSets[MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].SetIDX]
                        ...
                        for (var index5 = 0; index5 <= num4; ++index5)
                        {
                            if (DatabaseAPI.Database
                                    .EnhancementSets[
                                        MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].SetIDX]
                                    .Bonus[index4]
                                    .Index[index5] <= -1)
                            {
                                continue;
                    ...
                    var num5 = MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].EnhIndexes.Length -
                               1;
                    for (var index4 = 0; index4 <= num5; ++index4)
                    {
                        var index5 = DatabaseAPI.IsSpecialEnh(MidsContext.Character.CurrentBuild.SetBonuses[index1]
                            .SetInfo[index2].EnhIndexes[index4]);
                        if (index5 > -1)
                        {
                            iText = iText + ListItemOn() + formatColor("  " + DatabaseAPI.Database.EnhancementSets[MidsContext.Character.CurrentBuild.SetBonuses[index1].SetInfo[index2].SetIDX].GetEffectString(index5, true, true), ExportConfig.Element.Power) + ListItemOff();
                        }
                    }
