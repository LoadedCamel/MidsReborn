...
namespace Mids_Reborn.UI.Controls
{
    ...
    public partial class MidsDataViewNeo : UserControl
    {
        ...
        private readonly record struct DamageSourceEstimate(string Label, float Total);
        private readonly record struct DamageCompositionEstimate(
            float LocalSlotting,
            float SetBonuses,
            ...
            IReadOnlyList<DamageSourceEstimate> TopSources)
        {
            ...
            public bool HasAny =>
                ...
                Math.Abs(SetBonuses) > float.Epsilon ||
        ...
        private DamageCardPresentation BuildDamageCardPresentation(IPower basePower, IPower enhancedPower)
        {
            ...
            var nakedSummary = ResolveDamageStageSummary(
                powerSnapshot?.BasePower,
                historyIndex >= 0 ? MainModule.MidsController.Toon?.GetBasePower(historyIndex) : null);
            var preBuffSummary = ResolveDamageStageSummary(
                powerSnapshot?.PreBuffPower,
                powerSnapshot?.MathPower ?? enhancedPower);
            return new DamageCardPresentation(
                HeaderText: "Damage",
                ...
                TooltipText: GetDamageTooltipText(historyIndex, enhancedPower, enhancedSummary, nakedSummary, preBuffSummary),
                Segments: segments);
        }
        ...
        private static string GetDamageTooltipText(
            int historyIndex,
            IPower displayedPower,
            DamageBreakdownSummary enhancedSummary,
            DamageBreakdownSummary nakedSummary,
            DamageBreakdownSummary preBuffSummary)
        {
            var compositionLines = BuildDamageCompositionLines(historyIndex, displayedPower, enhancedSummary, nakedSummary, preBuffSummary);
            var detailed = Power.FormatDamageTooltip(enhancedSummary, compositionLines);
            return string.IsNullOrWhiteSpace(detailed)
                ? BuildDamageTooltipText(enhancedSummary)
                : detailed;
        }

        private static IReadOnlyList<string> BuildDamageCompositionLines(
            int historyIndex,
            IPower displayedPower,
            DamageBreakdownSummary enhancedSummary,
            DamageBreakdownSummary nakedSummary,
            DamageBreakdownSummary preBuffSummary)
        {
            ...
            var nakedBase = Math.Max(0f, nakedSummary.TotalExcludingProc);
            var currentTotal = Math.Max(0f, enhancedSummary.DisplayedTotal);
            var directProcTotal = Math.Max(0f, enhancedSummary.DisplayedTotal - enhancedSummary.TotalExcludingProc);
            var compositionEstimate = EstimateDamageComposition(historyIndex, displayedPower, enhancedSummary, nakedSummary, preBuffSummary);
            var slottingTotal = Math.Max(0f, compositionEstimate.LocalSlotting);

            if (currentTotal > float.Epsilon)
            {
                lines.Add($"Current total: {DisplayValueFormatter.FormatNumber(currentTotal)}");
            }

            if (nakedBase > float.Epsilon)
            {
                lines.Add($"Base power with no slotting or buffs: {DisplayValueFormatter.FormatNumber(nakedBase)}");
            }
            ...
            if (slottingTotal > float.Epsilon || directProcTotal > float.Epsilon || compositionEstimate.HasAny)
            {
                ...
                {
                    lines.Add($"From set bonuses: +{DisplayValueFormatter.FormatNumber(compositionEstimate.SetBonuses)}");
                }

                if (compositionEstimate.IncarnateBonuses > float.Epsilon)
                {
                    lines.Add($"From incarnates: +{DisplayValueFormatter.FormatNumber(compositionEstimate.IncarnateBonuses)}");
                }

                if (compositionEstimate.OtherPowerBuffs > float.Epsilon)
                {
                    lines.Add($"From other power buffs: +{DisplayValueFormatter.FormatNumber(compositionEstimate.OtherPowerBuffs)}");
                }

                var otherBonuses = Math.Max(0f, compositionEstimate.OtherBonuses);
                if (otherBonuses > float.Epsilon)
                {
                ...
                {
                    lines.Add("Top contributing sources:");
                    foreach (var source in compositionEstimate.TopSources.Take(5))
                    {
                        lines.Add($"{source.Label}: +{DisplayValueFormatter.FormatNumber(source.Total)}");
                    }
                }
            }
            else
        ...
        private static DamageCompositionEstimate EstimateDamageComposition(
            int historyIndex,
            IPower displayedPower,
            DamageBreakdownSummary enhancedSummary,
            DamageBreakdownSummary nakedSummary,
            DamageBreakdownSummary preBuffSummary)
        {
            var contributionSnapshot = MainModule.MidsController.Toon?.LastCalculationSnapshot?.PlayerActorSnapshot.Contributions;
            var nakedBase = Math.Max(0f, nakedSummary.TotalExcludingProc);
            var currentNoProc = Math.Max(0f, enhancedSummary.TotalExcludingProc);
            var nonBaseDelta = Math.Max(0f, currentNoProc - nakedBase);

            if (historyIndex < 0 || contributionSnapshot == null || displayedPower == null)
            {
            ...
            var baseByType = nakedSummary.ByType
                .Where(contribution => Math.Abs(contribution.DisplayedTotal) > float.Epsilon)
                .ToDictionary(contribution => contribution.DamageType, contribution => contribution.DisplayedTotal);
            if (baseByType.Count == 0)
            {
                return new DamageCompositionEstimate(
                    nonBaseDelta,
                    0f,
            ...
            void AddSourceContribution(ContributionRecord record, float amount, bool fromChanceModifier)
            {
                ...
                var isDisplayedPowerSource = IsDisplayedPowerSource(record.Source, displayedPower);

                if (record.Source.IsSetBonusVirtual || record.Category == ContributionCategory.SetBonus)
                {
                    setBonuses += amount;
                }
                else if (record.Source.IsIncarnate)
                {
                ...
                else if (isDisplayedPowerSource)
                {
                    return;
                }
                ...
                if (isDisplayedPowerSource)
                {
                    return;
                }

                var label = ResolveContributionSourceLabel(record.Source);
                if (string.IsNullOrWhiteSpace(label))
                {
                    return;
                }

                if (topSources.TryGetValue(label, out var existing))
                {
                    topSources[label] = existing + amount;
                }
                else
                {
                    topSources[label] = amount;
                }
            }

            foreach (var record in contributionSnapshot.Records)
            {
                if (record.Key is { Bucket: ContributionBucket.Damage } damageKey &&
            ...
            var orderedTopSources = topSources
                ...
                .Select(item => new DamageSourceEstimate(item.Key, item.Value))
                .OrderByDescending(item => Math.Abs(item.Total))
                .ThenBy(item => item.Label, StringComparer.OrdinalIgnoreCase)
                .ToArray();

            var positiveSetBonuses = Math.Max(0f, setBonuses);
            ...
            {
                var normalization = nonBaseDelta / recognizedNonLocalTotal;
                positiveSetBonuses *= normalization;
        ...
        private static bool IsDisplayedPowerSource(ContributionSource source, IPower displayedPower)
        {
            if (displayedPower == null)
            {
                return false;
            }

            var displayName = displayedPower.DisplayName?.Trim() ?? string.Empty;
            var fullName = displayedPower.FullName?.Trim() ?? string.Empty;
            return (!string.IsNullOrWhiteSpace(fullName) &&
                    string.Equals(source.FullName, fullName, StringComparison.OrdinalIgnoreCase)) ||
                   (!string.IsNullOrWhiteSpace(displayName) &&
                    string.Equals(source.Name, displayName, StringComparison.OrdinalIgnoreCase));
        }

        private static bool IsOtherPowerBuffSource(ContributionRecord record, IPower displayedPower)
        {
            if (record.Category == ContributionCategory.SelfBuff ||
                record.Channel == ContributionChannel.Buffs)
            {
                return true;
            }

            if (record.Source.IsSetBonusVirtual || record.Source.IsIncarnate)
            {
                return false;
            }

            if (IsDisplayedPowerSource(record.Source, displayedPower))
            {
                return false;
            }

            return !string.IsNullOrWhiteSpace(record.Source.FullName) &&
                   !record.Source.IsPvpResist;
        }

        private static string ResolveContributionSourceLabel(ContributionSource source)
        {
            if (source.IsSetBonusVirtual)
            {
                return "Set bonuses";
            }

            if (source.IsPvpResist)
            {
                return "PvP resistance bonus";
            }

            return string.IsNullOrWhiteSpace(source.Name) ? string.Empty : source.Name;
        }

        private static bool TryGetChanceModifierTag(ContributionRecord record, out string chanceTag)
        {
            chanceTag = string.Empty;
            if (record.EffectType != Enums.eEffectType.GlobalChanceMod ||
                string.IsNullOrWhiteSpace(record.MetricName) ||
                !record.MetricName.StartsWith("ChanceModifier[", StringComparison.OrdinalIgnoreCase) ||
                !record.MetricName.EndsWith("]", StringComparison.Ordinal))
            {
                return false;
            }

            chanceTag = record.MetricName["ChanceModifier[".Length..^1].Trim();
            return !string.IsNullOrWhiteSpace(chanceTag);
        }

        private static float EstimateChanceModifierDamageContribution(
            IPower displayedPower,
            string chanceTag,
            float chanceDelta,
            float displayMultiplier)
        {
            if (string.IsNullOrWhiteSpace(chanceTag) || Math.Abs(chanceDelta) <= float.Epsilon)
            {
                return 0f;
            }

            var total = 0f;
            foreach (var effect in Power.GetIncludedDamageEffects(displayedPower))
            {
                if (!EffectUsesChanceTag(effect, chanceTag))
                {
                    continue;
                }

                var probability = effect.Probability;
                if (probability <= float.Epsilon)
                {
                    continue;
                }

                var currentContribution = Power.GetDamageEffectTotal(effect, displayedPower, absolute: true, applyReturnScaling: true);
                if (Math.Abs(currentContribution) <= float.Epsilon)
                {
                    continue;
                }

                total += (currentContribution / probability) * chanceDelta;
            }

            return total * displayMultiplier;
        }

        private static bool EffectUsesChanceTag(IEffect effect, string chanceTag)
        {
            if (!string.IsNullOrWhiteSpace(effect.EffectId) &&
                string.Equals(effect.EffectId, chanceTag, StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        private DamageBreakdownSummary ResolveDamageStageSummary(IPower? preferredPower, IPower? fallbackPower)
        {
            var sourcePower = preferredPower ?? fallbackPower;
            if (sourcePower == null)
            {
                return DamageBreakdownSummary.Empty;
            }

            var summary = Power.GetDamageBreakdown(sourcePower);
            if (_presentationMode == MidsDataViewNeoPresentationMode.ActorReadOnly && _actorTotalsSnapshot != null)
            {
                var actorHpMax = Math.Max(1f, _actorTotalsSnapshot.Totals.HPMax);
                summary = summary.ScaleToDisplayMultiplier(actorHpMax);
            }
