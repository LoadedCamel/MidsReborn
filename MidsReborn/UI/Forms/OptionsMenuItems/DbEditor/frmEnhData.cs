...
namespace Mids_Reborn.UI.Forms.OptionsMenuItems.DbEditor
{
    public partial class frmEnhData : Form
    {
        ...
        private void btnImage_Click(object sender, EventArgs e)
        {
            if (Loading)
                return;
            ImagePicker.InitialDirectory = AssetManager.GetDbEnhancementsPath();
            ...
            if (!File.Exists(Path.Combine(AssetManager.GetDbEnhancementsPath(), imageFile)) && !File.Exists(Path.Combine(AssetManager.GetEnhancementsPath(), imageFile)))
            {
                MessageBox.Show($@"You must select an image from either the {AssetManager.GetEnhancementsPath()} or the {AssetManager.GetDbEnhancementsPath()} folder!\r\n\r\nIf you are adding a new image, you should copy it to the appropriate folder and then select it.", @"Select Image", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
        ...
        private void DisplayIcon()
        {
            if (!string.IsNullOrWhiteSpace(myEnh.Image))
            {
                ...
                using var extendedBitmap2 = new ExtendedBitmap(48, 48);

                if (extendedBitmap2.Graphics != null)
                {
                    // Look up the correct border from the dictionary
                    var grade = AssetManager.ToGfxGrade(myEnh.TypeID);
                    var borderKey = new Point(AssetManager.OriginIndex, (int)grade);
        ...
        {
            if (myEnh.nIDSet <= -1) return;

            myEnh.Image = DatabaseAPI.Database.EnhancementSets[myEnh.nIDSet].Image;
            DisplayIcon();
            SetTypeIcons();
            if (string.IsNullOrWhiteSpace(DatabaseAPI.Database.EnhancementSets[myEnh.nIDSet].Image)) return;
            ...
            // Look up the specific "SetO" border
        ...
        private void DrawClasses()
        {
            ...
            bxClass.Graphics.FillRectangle(solidBrush, bxClass.Graphics.ClipBounds);

            foreach (var classId in myEnh.ClassID)
            {
                ...
                {
                    bxClass.Graphics.DrawImage(classIcon.Bitmap, destRect);
                }
        ...
        private void SetTypeIcons()
        {
            ...
            using var extendedBitmap1 = new ExtendedBitmap(48, 48);
            using var extendedBitmap2 = string.IsNullOrWhiteSpace(img)
                ? new ExtendedBitmap(48, 48)
                : new ExtendedBitmap(path);

            if (extendedBitmap1.Graphics == null) return;

            extendedBitmap1.Graphics.Clear(Color.Transparent);
            extendedBitmap1.Graphics.InterpolationMode = InterpolationMode.HighQualityBicubic;
            extendedBitmap1.Graphics.SmoothingMode = SmoothingMode.HighQuality;
            extendedBitmap1.Graphics.PixelOffsetMode = PixelOffsetMode.HighQuality;

            // Helper function to look up and draw a border
            void DrawBorder(Enums.eType type)
            {
                var grade = AssetManager.ToGfxGrade(type);
                var key = new Point(AssetManager.OriginIndex, (int)grade);
                if (AssetManager.Borders.TryGetValue(key, out var borderImage) && borderImage?.Bitmap != null)
                {
                    extendedBitmap1.Graphics.DrawImage(borderImage.Bitmap, extendedBitmap1.Graphics.ClipBounds);
                }
            }
