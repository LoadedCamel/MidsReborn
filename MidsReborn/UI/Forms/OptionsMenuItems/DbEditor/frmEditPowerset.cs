...
namespace Mids_Reborn.UI.Forms.OptionsMenuItems.DbEditor
{
    public partial class frmEditPowerset : Form
    {
        ...
        private void btnIcon_Click(object sender, EventArgs e)
        {
            ...
            ImagePicker.InitialDirectory = AssetManager.GetDbPowerSetsPath();
        ...
        {
            var powersetIcon = AssetManager.GetPowersetImage(MyPowerSet);

            if (powersetIcon?.Bitmap != null && powersetIcon != AssetManager.UnknownIcon)
            {
                picIcon.Image = new Bitmap(powersetIcon.Bitmap);
                btnIcon.Text = MyPowerSet.ImageName;
            }
            else
            {
                // Fallback if the icon is the generic 'Unknown' or null.
                if (AssetManager.UnknownIcon?.Bitmap != null)
                {
                    picIcon.Image = new Bitmap(AssetManager.UnknownIcon.Bitmap);
                }
                else
                {
