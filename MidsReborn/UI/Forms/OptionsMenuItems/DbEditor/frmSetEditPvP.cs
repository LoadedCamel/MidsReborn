    ...
    public partial class FrmSetEditPvP : Form
    {
        ...
        private void DisplayIcon()
        {
            if (!string.IsNullOrWhiteSpace(MySet.Image))
            {
                var img = MySet.Image;
                string path;
                if (File.Exists(Path.Combine(AssetManager.GetEnhancementsPath(), img)))
                {
                    path = Path.Combine(AssetManager.GetEnhancementsPath(), img);
                }
                else if (File.Exists(Path.Combine(AssetManager.GetDbEnhancementsPath(), img)))
                {
                    path = Path.Combine(AssetManager.GetDbEnhancementsPath(), img);
                }
                else
                {
                    // If the specific image can't be found, we'll fall back to the cached Unknown icon.
                    path = null;
                }

                // Use the found image path, or the cached Unknown icon as a fallback.
                using var imageToDraw = (path != null) ? new ExtendedBitmap(path) : AssetManager.UnknownIcon;
            ...
            {
                // This is the case for a new set with no image assigned yet.
                using var canvas = new ExtendedBitmap(48, 48);
        ...
        private void FillImageList()
        {
            ...
            if (extendedBitmap.Graphics == null) return;

            ilEnh.Images.Clear();
            foreach (var enhIndex in MySet.Enhancements)
            {
                ...
                if (enhancement.ImageIdx > -1)
                {
                    try
                    {
                        var gfxGrade = AssetManager.ToGfxGrade(enhancement.TypeID);
                        extendedBitmap.Graphics.Clear(Color.Transparent);
                        var graphics = extendedBitmap.Graphics;
                        var destRect = new Rectangle(0, 0, width1, height1);

                        AssetManager.DrawEnhancementAt(graphics, destRect, enhancement.ImageIdx, gfxGrade);
                    ...
                    {
                        // Use the pre-cached UnknownPowerset bitmap directly
                        if (AssetManager.UnknownIcon?.Bitmap != null)
                        {
                            ilEnh.Images.Add(new Bitmap(AssetManager.UnknownIcon.Bitmap));
                        }
                    }
                }
                else
