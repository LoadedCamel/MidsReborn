...
namespace Mids_Reborn.UI.Forms
{
    public partial class SetInspector : Form
    {
        ...
        private void FillImageList(bool filtered = false)
        {
            using var extendedBitmap = new ExtendedBitmap(setImageList.ImageSize);
            setImageList.Images.Clear();
            if (!filtered)
                for (var index = 0; index < EnhancementSetList.Count; index++)
                {
                    var set = EnhancementSetList[index];
                    if (set.ImageIdx > -1)
                    {
                        ...
                        // Use the correct DrawEnhancementSet method with a destination rectangle and no 'ref'
                        AssetManager.DrawEnhancementSet(graphics, destRect, index);
            ...
            else
            {
                ...
                for (var index = 0; index < _filteredEnhancementSets.Count; index++)
                {
                    var set = _filteredEnhancementSets[index];
                    if (set.ImageIdx > -1)
                    {
                        extendedBitmap.Graphics!.Clear(Color.Transparent);
                        var graphics = extendedBitmap.Graphics;
        ...
        private async void FillPowerImageList()
        {
            ...
            foreach (var power in _baseFilteredPowers)
            {
                ...
                // Use the helper method to get the pre-cached image
                ...
                {
                    // Add the generic unknown icon as a fallback
