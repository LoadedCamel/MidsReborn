        ...
        private void DisplayData()
        {
            ...
            for (var i = 0; i < PData.Sections.Length; i++)
            {
                for (var j = 0; j < PData.Sections[i].Content.Length; j++)
                {
                    ...
                    var color = PData.Sections[i].Content[j].Color switch
                    {
                        _ when lineText.Contains("[Empty]") => RTF.Color(RTF.ElementID.Faded),
                        _ when PData.Sections[i].Content[j].Color == Color.FromArgb(0, 255, 128) => RTF.Color(RTF.ElementID.Enhancement),
                        _ when PData.Sections[i].Content[j].Color == Color.FromArgb(0, 255, 255) => RTF.Color(RTF.ElementID.Invention),
                        _ => ""
                    };
                    ...
                    text += $"{color}{new string(' ', PData.Sections[i].Content[j].Indent * indentMultiplier)}{lineText}{(color != "" ? RTF.Color(RTF.ElementID.Text) : "")}";
                }
            }
