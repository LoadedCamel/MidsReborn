...
namespace Mids_Reborn.UI.Forms
{
    public partial class MainWindow2 : Form
    {
        ...
        private MidsDataViewNeo? myDataView => dataView;
        ...
        protected override void OnSizeChanged(EventArgs e)
        {
            ...
            // canvas.Invalidate();// re-blit the visible slice
        }
        ...
        private void InitializePopup()
        {
            _popupHost = new MidsPopupDisplay
            {
                ...
                ColumnRight = false,
                Font = new Font("Noto Sans", 9.25f, FontStyle.Regular, GraphicsUnit.Point),
        ...
        private void UpdateUiControls(bool suppressDuringSetup = false, bool skipResize = false)
        {
            ...
            using (_events.SuppressIf(suppressDuringSetup))
            {
                ...
                Debug.WriteLine($"Character Origin = {MidsContext.Character.Origin}");
        ...
        private void UpdateUiLayout(bool force = false)
        {
            ...
            // Apply scale, then recompute layout with the SAME width we used for scale
            drawing.MasterScale = master;
            drawing.UpdateFontScale(master);
            drawing.UpdateLayout(widthForLayout);

            // Rebuild backbuffer for the new size/scale and blit
            drawing.ReInit(canvas);     // BuildRenderer already does FullRedraw at the end of ReInit
            canvas.ResizeToContent();   // adopt the new content height
            canvas.Invalidate();
        }
        ...
        private void GetBestDamageValues()
        {
            ...
            myDataView.infoDamageDisplay.HighestBaseValue = highBase;
            myDataView.infoDamageDisplay.HighestEnhancedValue = highEnh;
        }
        ...
        private void UpdateToon(clsToonX toon, Character? ch, int primaryIndex, int secondaryIndex, int pool0Index, int pool1Index, int pool2Index, int pool3Index, int ancillaryIndex, Func<Archetype, Enums.ePowerSetType, IPowerset[]> getPowerSets, Action lockSecondary)
        {
            var at = ch.Archetype;
