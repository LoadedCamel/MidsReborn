...
namespace Mids_Reborn.UI.Forms
{
    public partial class frmSetFind : Form
    {
        ...
        public frmSetFind(MainWindow iParent)
        {
        ...
        private void FillImageList()
        {
            ...
            foreach (var set in DatabaseAPI.Database.EnhancementSets)
            {
                if (set.ImageIdx > -1)
                {
                    ...
                    AssetManager.DrawEnhancementSet(graphics, set.ImageIdx);
        ...
        private void FillMatchingPowers(int sIdx)
        {
            ...
            foreach (var powerset in matchingPowersets)
            {
                ...
                using (var g = Graphics.FromImage(icon))
                {
                    if (powerset != null)
                    {
                        // Use the GetPowersetImage helper method to get the icon
                        var psIcon = AssetManager.GetPowersetImage(powerset);
                        if (psIcon?.Bitmap != null)
                        {
                            g.DrawImage(psIcon.Bitmap, new Rectangle(2, 0, 16, 16));
                        }
                    }
                }
                imgList.Images.Add(icon);
                powerSetsIconsDict.Add(powerset.FullName, imgIdx++);
            }

            for (var i = 0; i < matchingArchetypes.Count + 1; i++)
            {
                var isUnknown = (i == matchingArchetypes.Count);
                var idx = isUnknown ? -1 : Array.IndexOf(DatabaseAPI.Database.Classes, matchingArchetypes[i]);
                var icon = new Bitmap(18, 16);
                using (var g = Graphics.FromImage(icon))
                {
                    if (!isUnknown && AssetManager.Archetypes.TryGetValue(idx, out var atIcon) && atIcon?.Bitmap != null)
                    {
                        g.DrawImage(atIcon.Bitmap, new Rectangle(0, 0, 16, 16));
                    }
                    else if (AssetManager.UnknownIcon?.Bitmap != null)
                    {
                        g.DrawImage(AssetManager.UnknownIcon.Bitmap, new Rectangle(0, 0, 16, 16));
                    }
                }
                imgList.Images.Add(icon);
                var className = isUnknown ? "unknown_generic" : matchingArchetypes[i].ClassName;
                atIconsDict.Add(className, imgIdx++);
            }
